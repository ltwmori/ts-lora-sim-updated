...
class EndNode(NetworkNode):
	sf_distribution = {7: 5, 8: 4, 9: 4}  # The distribution of SFs
	sf_counter = {7: 0, 8: 0, 9: 0}  # Counter for the number of nodes with each SF
	def __init__(self, node_id):
		super().__init__(node_id)
	...
	def assign_sf(self):
		for sf, target_count in self.sf_distribution.items():
			if self.sf_counter[sf] < target_count:
				self.sf_counter[sf] += 1
				return sf
		print(f"WARNING: {self} could not be assigned an SF!")
		return None
...
def show_final_statistics():
	...
	print("Join Collisions:", nr_collisions)
	...
	output_file.write("Join Collisions: " + str(nr_collisions) + "\n"
					  + "Data collisions: " + str(nr_data_collisions) + "\n"
					  + "Lost packets: " + str(nr_lost) + "\n"
					  + "Transmitted data packets: " + str(nr_data_packets_sent) + "\n"
					  + "Transmitted SACK packets: " + str(nr_sack_sent) + "\n"
					  + "Transmitted join request packets: " + str(nr_join_req_sent) + "\n"
					  + "Transmitted join accept packets: " + str(nr_join_acp_sent) + "\n"
					  + "Data Retransmissions: " + str(nrRetransmission) + "\n"
					  + "Join request packets dropped by gateway: " + str(nr_join_req_dropped) + "\n"
					  + "Average join time: " + str(avr_join) + " s\n"
					  + "Erx: " + str(erx) + " J\n"
					  + "Etx: " + str(etx) + " J\n"
					  + "Average energy consumption per node: " + str(total_energy / nodes_count) + " J\n"
					  + "Number of nodes failed to connect to the network: " + str(nodes_count - nr_joins) + "\n")
